...
def parse_arguments():
    ...
    p.add_argument('-c', '--config_file', help='Path to config file.', default='/etc/secura_pi_cam.conf')
...
def take_photo():
    ...
    """
    camera_output_file = config['image_path'] + "/secura_pi_cam-" + datetime.now().strftime("%Y-%m-%d-%H%M%S") + ".jpg"
    if args.debug:
        GPIO.output(32,True)
        time.sleep(0.25)
        GPIO.output(32,False)
    with picamera.PiCamera() as config['camera']:
...
def monitor_alarm_state():
    ...
    def update_alarm_state(new_alarm_state):
        if new_alarm_state != alarm_state['current_state']:
            ...
            logger.info("secura_pi_cam is now %s" % alarm_state['current_state'])
            telegram_send_message('secura_pi_cam: %s' % alarm_state['current_state'])
    while True:
        time.sleep(5)
        now = time.time()
        if now - last_telegram_update > 180:
            ...
            if len(messages) < 1:
                ...
                if 'status' in last_telegram_message.message.text.lower():
                    ...
                    if message_id not in status_replied:
                        telegram_send_message('secura_pi_cam status: %s' % prepare_status(alarm_state))
                        status_replied[message_id] = True
        if alarm_state['current_state'] != 'disabled':
            if now - alarm_state['last_packet'] > 120:
                update_alarm_state('armed')
            elif now - alarm_state['last_packet'] > 200:
                arp_ping_macs()
            else:
                update_alarm_state('disarmed')
...
def clean_exit(signal = None, frame = None):
    logger.info("secura_pi_cam stopping...")
    GPIO.cleanup()
    sys.exit(0)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("secura_pi_cam running")
        telegram_send_message('secura_pi_cam running')
        while 1:
            time.sleep(100)
